...
class ModelTrain():
    def __init__(self, model_name, batch_size, epochs, test_size=0.2, model_arq=None, lr=1.0, dlr=0.0):
    ...
    def log_confusion_matrix(self):
        ...
        ConfusionMatrix(logdir=self.logdir).save_cm(
            cm, class_names=app_cfg['class_names'])
    ...
    def compile(self, loss_name='categorical_crossentropy', metrics=['accuracy'], optimizer_name='adam'):
        logger.info(
            f"Compiling model. (name_loss={loss_name}, metrics={metrics}, optimizer_name={optimizer_name})")

        # Optimizer
        if optimizer_name == 'adam':
            optimizer = keras.optimizers.Adam(
                lr=self.learning_rate, decay=self.decay_learning_rate)
        elif optimizer_name == 'rmsprop':
            optimizer = keras.optimizers.RMSprop(
                lr=self.learning_rate, decay=self.decay_learning_rate)
        elif optimizer_name == 'sgd':
            optimizer = keras.optimizers.SGD(
                lr=self.learning_rate, decay=self.decay_learning_rate, momentum=0.9, nesterov=None)
        else:
            raise RuntimeError(
                'Optimizer should be one of Adam, RMSprop and SGD.')
        self.model.compile(
            loss=loss_name, optimizer=optimizer, metrics=metrics)

    def model_train(self, batch_size, epochs):
        logger.info(
            f"Starting model training. [batch_size={batch_size}, epochs={epochs}]")
        start_time = datetime.now()
        history = self.model.fit(self.train_data, self.train_label, batch_size=batch_size, validation_data=(
            self.test_data, self.test_label), epochs=epochs, callbacks=[self.tb])
